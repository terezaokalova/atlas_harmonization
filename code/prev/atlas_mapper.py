...
class AtlasMapper:
    def __init__(self, roi_info):
        """
        Initialize the AtlasMapper with ROI information.

        Args:
            roi_info (DataFrame): DataFrame containing ROI indices and coordinates.
        """
        self.roi_info = roi_info
        self.roi_coords = roi_info[['x', 'y', 'z']].values
        self.roi_labels = roi_info['Sno'].values  # Use 'Sno' instead of 'parcel'

    def map_electrodes_to_rois(self, electrode_coords, patient_nums):
        """
        Map electrodes to the nearest ROI based on coordinates.
        ...
        """
        nbrs = NearestNeighbors(n_neighbors=1).fit(self.roi_coords)
